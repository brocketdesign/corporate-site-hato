# 合同会社はと

最先端のAI技術で、ビジネスの未来を創造するデジタルソリューション企業です。

## 企業概要

合同会社はとは、AI技術を核としたデジタルソリューションを提供するテクノロジー企業です。最新の技術と創造的な発想で、お客様のビジネス課題を解決し、新たな価値を創造します。

### 実績
- **150+** 導入企業
- **98%** 顧客満足度
- **5+** 年の実績

## サービス

### AIアプリケーション開発
機械学習と深層学習を活用した、業界特化型AIアプリケーションの開発

### データ分析・予測
ビッグデータを活用した洞察と、正確な将来予測で意思決定を支援

### システムインテグレーション
既存システムとAIのシームレスな連携で、スムーズなデジタル変革を実現

### コンサルティング
AI導入戦略の立案から運用サポートまで、包括的なコンサルティング

## プロダクト

### 生成 (Seisei) - AI Creative Platform
AIが生み出す、無限のクリエイティブ
- テキストから画像生成
- 自動文章作成
- マルチリンガル対応

🔗 [https://www.seisei.me](https://www.seisei.me)

### 楽アド (Rakuado) - Ad Network Platform
広告運用を、もっと簡単に
- 自動入札調整
- パフォーマンス分析
- 予算最適化

🔗 [https://app.rakuado.net](https://app.rakuado.net)

## 技術スタック

- React
- TypeScript
- Vite
- Tailwind CSS

## 開発

```bash
# 依存関係のインストール
npm install

# 開発サーバーの起動
npm run dev

# ビルド
npm run build
```

## 📧 メール送信設定

お問い合わせフォームからのメール送信には、Zoho Mail SMTPを使用しています。

### セットアップ手順

1. [Zoho Mail](https://mail.zoho.com) でアプリケーションパスワードを生成
2. `.env.example` をコピーして `.env` を作成
3. 環境変数を設定（`EMAIL_SETUP.md` を参照）
4. Vercel/Netlify または独自サーバーにデプロイ

詳細は [EMAIL_SETUP.md](./EMAIL_SETUP.md) をご覧ください。

## 本店住所

〒530-0001  
大阪府大阪市北区梅田1-2-2  
大阪駅前第2ビル12-12

## お問い合わせ

サービスに関するお問い合わせは、ウェブサイトのお問い合わせフォームからお願いいたします。

---

© 2026 合同会社はと. All rights reserved.
